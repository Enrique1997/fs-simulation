...
{   
    ...
    do
    {
        ...
        else if (exe < itemCount)
        {
            ...
            // duplicate string
            strcpy(copy[exe], input[exe]);

            // tokenise string
            len = tokenise(copy[exe], tok); 
            
            // ADDING
            if (strcmp(tok[0], "add") == 0)
            {
                // determine number of blocks needed
                blkCount = ((len - 1) / 3) + ((len - 1) % 3);
                // printf("%d block(s) are needed to hold %d file data\n", blkCount, len - 1);

                // array of available blocks
                int blkArray[blkCount];
